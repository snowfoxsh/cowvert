    ...
    F: FnOnce(T) -> (T, R),
{
    ...
    unsafe {
        ...
        let (new_t, result) = closure(old_t);
...
///
/// # Important
/// Will abort the program if the closure panics. This version explicit catches the panic.
///
/// # Example
/// ```
/// struct Foo;
...
/// // assert_eq!(message, "Done");
/// ```
pub fn take_catch_unwind<T, F, R>(mut_ref: &mut T, closure: F) -> R
where
    F: FnOnce(T) -> (T, R),
{
    use std::ptr;
